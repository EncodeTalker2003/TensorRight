...
-- axis relabling mapping for the 'TensorRight.Internal.DSL.DSL.relabel' operator.
...
-- | t'RelabelMapDesc' describes a single relabling in a relabel map, which
-- consists of an 'RClassRef' and another 'RClassRef'. A t'RelabelMapDesc' can be
...
-- | TensorRight DSL uses a list of t'RelabelMapDesc' to represent relabel maps
instance IsRelabelMap [RelabelMapDesc] where
